  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-white border-b border-black">
      <div className="relative flex items-center px-2 sm:px-4">
        <div className="absolute left-0 px-2 sm:px-4 py-2 text-xs sm:text-sm border-r border-black">
          {currentTime}
        </div>
        ...
        <nav className="absolute right-0 border-l border-black">
          <ul className="flex text-xs sm:text-sm">
            {["WORK", "ABOUT", "CONTACT"].map((item) => (
              <li key={item} className="border-r border-black last:border-r-0">
                <Link
                  href={`#${item.toLowerCase()}`}
                  className="px-2 sm:px-4 py-2 hover:underline block"
                >
                  {item}
                </Link>
              </li>
